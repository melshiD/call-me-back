...
**Last Updated:** 2025-12-04 06:15 EST
**Status:** Living Document
**Tags:** [voice-pipeline, architecture, real-time-streaming, ai-services, deepgram-flux]
...
2. [Deepgram Flux Turn-Taking](#deepgram-flux-turn-taking) *(NEW - 2025-12-04)*
3. [Services Integration](#services-integration)
4. [Call Flow](#call-flow)
5. [Debugging Guide](#debugging-guide)
6. [Performance Tuning](#performance-tuning)
7. [VAD & Turn Detection](#vad--turn-detection)
8. [Research & Future Work](#research--future-work)
...
The voice pipeline runs on a **Vultr VPS ([VULTR_VPS_IP])** instead of Cloudflare Workers due to hard technical constraints:
...
└────────────────────┬────────────────────────────────────────┘
                     ...
                     ▼
┌─────────────────────────────────────────────────────────────┐
│ VULTR VPS ([VULTR_VPS_IP]) - Node.js Voice Pipeline         │
│                                                              │
│  ┌─────────────────────────────────────────────────────┐   │
...
**Status:** DEPLOYED (2025-12-01)
**Impact:** Native turn-taking replaced timer-based silence detection

### What is Deepgram Flux?

Deepgram Flux is an advanced speech-to-text model with **native turn-taking detection**. Unlike the previous Nova-2 approach (which required timer-based silence detection and LLM evaluation), Flux provides:

- **EndOfTurn events** - Deepgram tells us when user has finished speaking
- **EagerEndOfTurn events** - Early speculation that user might be done
- **TurnResumed events** - User continued after an EagerEndOfTurn

### Why This Matters

**Before Deepgram Flux (Nov 2025):**
- Silence detection: 800-1200ms timer
- LLM evaluation: "Is user done speaking?"
- Total latency: 2-4 seconds
- False positives/negatives common

**After Deepgram Flux (Dec 2025):**
- Native EndOfTurn detection: ~200-500ms
- No LLM evaluation needed for turn detection
...
```javascript
// Previous (Nova-2)
const deepgramUrl = 'wss://api.deepgram.com/v1/listen?model=nova-2&language=en-US';

// Current (Flux)
const deepgramUrl = 'wss://api.deepgram.com/v1/listen?model=flux-general-en';
```

**TurnInfo Event Handling:**
```javascript
// In handleFluxTurnInfo()
handleFluxTurnInfo(data) {
  const turnInfo = data.turn_info;

  if (turnInfo.type === 'EndOfTurn') {
    // User definitively finished speaking
    // Trigger AI response immediately
    this.triggerResponse('flux_end_of_turn');
  }
  else if (turnInfo.type === 'EagerEndOfTurn') {
    // Deepgram thinks user might be done
    // Start speculative processing (optional)
    this.startSpeculativeResponse();
  }
  else if (turnInfo.type === 'TurnResumed') {
    // User continued after EagerEndOfTurn
    // Cancel speculative processing
    this.cancelSpeculativeResponse();
  }
}
```

### Speculative Response Pattern

With EagerEndOfTurn events, we can start processing before the user is definitely done:

```
Timeline:
T=0ms     User speaking
T=500ms   [EagerEndOfTurn] - Start speculative LLM inference
T=700ms   [TurnResumed] - Cancel inference, user still speaking
T=1200ms  [EagerEndOfTurn] - Start speculative LLM inference
T=1500ms  [EndOfTurn] - Confirm response, send to TTS
T=1800ms  User hears first audio
```

This reduces perceived latency by starting LLM inference during the natural pause at the end of user speech.

### Configuration

**Current Settings (index.js):**
```javascript
...
const fluxConfig = {
  ...
  turn_detection: {
    enabled: true,
    type: 'semantic'  // Uses AI to understand turn boundaries
  }
};
```

### Debugging Flux Events

```bash
# Monitor Flux turn events
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline | grep -i 'turn\|flux'"

# Check for EndOfTurn triggers
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline | grep -E 'EndOfTurn|EagerEndOfTurn|TurnResumed'"
```

### User Feedback

From session log 2025-12-01_03-10:
> "OH MY GOD! It's so much better!"

The improvement in conversational flow was immediately noticeable.

### Related Files

- `voice-pipeline-nodejs/index.js` - `handleFluxTurnInfo()` function
- Session log: `NEXT_SESSION_LOG_2025-12-01_03-10_DEEPGRAM_FLUX_INTEGRATION.md`
...
**Role:** Real-time streaming transcription, interim results, confidence scoring, **native turn-taking**

**Integration:**
- **API:** Streaming WebSocket (wss://api.deepgram.com/v1/listen)
- **Model:** `flux-general-en` (Deepgram Flux - with native turn detection)
- **Previous Model:** nova-2 (deprecated for this project)
- **Language:** en-US
- **Cost:** $0.0059/minute ($0.03 per 5-min call)
- **Latency:** 200-500ms (real-time streaming)
- **Turn Detection:** Native EndOfTurn events (see [Deepgram Flux section](#deepgram-flux-turn-taking))
...
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline | grep -i deepgram | head -20"

# Check WebSocket state
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline | grep -E 'readyState|CONNECTING|OPEN'"
```
...
**Role:** Response generation (turn evaluation now handled by Deepgram Flux)

**Specs:**
- **Models Available:**
  - `llama3.1-8b` - Fast, cheap ($0.10/1M tokens), good for most personas
  - `llama3.3-70b` - Smarter, better reasoning ($0.60/1M tokens), for complex personas
- **Per-Persona Selection:** Database `personas.llm_model` column specifies which model to use
- **API:** RESTful HTTP (not WebSocket)
- **Latency:** <1 second per inference (both models, 2000+ tokens/sec)
- **Architecture:** Inference-only (no fine-tuning, pre-trained)

**Model Selection (2025-12-02):**
```javascript
// In generateResponse()
const modelId = persona.llm_model || 'llama3.1-8b';  // Default to 8B
...
**Primary Use Case: Response Generation**

> **Note:** Turn evaluation previously used Cerebras but is now handled natively by Deepgram Flux EndOfTurn events (see [Deepgram Flux section](#deepgram-flux-turn-taking)).
...
ssh root@[VULTR_VPS_IP] "grep -n 'eleven_.*_v2' /opt/voice-pipeline/index.js | head -5"

# Expected: eleven_turbo_v2_5 or eleven_flash_v2_5 (NOT turbo_v2 or flash_v2)

# Check if custom voice IDs are set
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline | grep -i 'voice\|ElevenLabs' | head -20"
```
...
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline --lines 500 | grep -c 'audio:'"

# Step 2: Verify ElevenLabs is generating audio
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline | grep -i 'elevenlabs\|audio' | head -30"

# Step 3: Check mulaw conversion function
ssh root@[VULTR_VPS_IP] "grep -n 'mulawToPcm\|pcmToMulaw\|ulaw' /opt/voice-pipeline/index.js | head -10"
...
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline --lines 500 | grep -i 'evaluation\|respond'"

# Check Deepgram connection status
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline | grep -i 'deepgram\|readystate\|connecting'"

# Check for errors
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline --err --lines 50"

# Monitor connection state
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline --lines 0 | grep -i 'error\|fail\|exception'"
```
...
```bash
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline --lines 0 --raw"
```

**Search for specific call:**
```bash
# Get call ID from frontend/logs
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline --lines 1000 | grep 'CA123ABC...'"
```

**Check specific service health:**
```bash
# Deepgram
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline --lines 500 | grep -i deepgram"

# ElevenLabs
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline --lines 500 | grep -i elevenlabs"

# Cerebras
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline --lines 500 | grep -i cerebras"

# Silero VAD
ssh root@[VULTR_VPS_IP] "pm2 logs voice-pipeline --lines 500 | grep -i 'vad\|silero'"
```
...
```bash
ssh root@[VULTR_VPS_IP] "ps aux | grep -i voice-pipeline"
ssh root@[VULTR_VPS_IP] "pm2 status"
ssh root@[VULTR_VPS_IP] "pm2 describe voice-pipeline"
```
