...
CREATE INDEX IF NOT EXISTS idx_cost_events_call ON call_cost_events(call_id);
CREATE INDEX IF NOT EXISTS idx_cost_events_breakdown ON call_cost_events(call_cost_breakdown_id);

-- Add foreign key constraints to existing tables (if not already added)
-- This ensures referential integrity
ALTER TABLE calls ADD CONSTRAINT IF NOT EXISTS fk_calls_user
    FOREIGN KEY (user_id) REFERENCES users(id);

ALTER TABLE scheduled_calls ADD CONSTRAINT IF NOT EXISTS fk_scheduled_calls_user
    FOREIGN KEY (user_id) REFERENCES users(id);

ALTER TABLE user_credits ADD CONSTRAINT IF NOT EXISTS fk_user_credits_user
    FOREIGN KEY (user_id) REFERENCES users(id);

ALTER TABLE credit_transactions ADD CONSTRAINT IF NOT EXISTS fk_credit_transactions_user
    FOREIGN KEY (user_id) REFERENCES users(id);

ALTER TABLE user_persona_relationships ADD CONSTRAINT IF NOT EXISTS fk_relationships_user
    FOREIGN KEY (user_id) REFERENCES users(id);

ALTER TABLE call_cost_breakdowns ADD CONSTRAINT IF NOT EXISTS fk_cost_breakdowns_user
    FOREIGN KEY (user_id) REFERENCES users(id);
...
VALUES (
    'demo_user',
    2000, -- $20 max per call for demo
    FALSE -- Don't cut off demo calls
